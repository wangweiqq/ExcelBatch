...
QString GetExcelCellStr(int val) {
	if (val - 1 < 26) {
		return QChar((val - 1) + 'A');
	}
	int n1 = (val - 1) / 26;
	int n2 = (val - 1) % 26;
	return QString("%1%2").arg(GetExcelCellStr(n1)).arg(QChar(n2 + 'A'));
}
QVariantList ExcelOperator::getRangeValues(QAxObject* pSheet, int startRow, int startCols, int endRow, int endCols) {
	QString range = QString("%1%2:%3%4").arg(GetExcelCellStr(startCols)).arg(startRow).arg(GetExcelCellStr(endCols)).arg(endRow);
	QAxObject *user_range = pSheet->querySubObject("Range(const QString&)", range);
	QVariant var = user_range->dynamicCall("Value");
	//QVariantList list = 
	return var.toList();
}
...
bool ExcelOperator::setRangeValues(QAxObject* pSheet, int startRow, int startCols, int endRow, int endCols, QList<QList<QVariant> >& cells) {
	QVariantList vars;
	const int rows = cells.size();
	int cols = endCols - startCols + 1;
	for (int i = 0; i<rows; ++i)
	{
		QList<QVariant>& colList = cells[i];
		while (colList.count() < cols) {
			colList.append(QVariant());
		}
		vars.append(QVariant(colList));
	}
	QVariant ttt = QVariant(vars);
	QString range = QString("%1%2:%3%4").arg(GetExcelCellStr(startCols)).arg(startRow).arg(GetExcelCellStr(endCols)).arg(endRow);
	qDebug() << "range = " << range;
	try {
		QAxObject *user_range = pSheet->querySubObject("Range(const QString&)", range);
		user_range->setProperty("Value", QVariant(vars));
	}
	catch (...) {
		qDebug() << "批量写入错误： " << range;
		return false;
...
/*设置单元格值*/
bool ExcelOperator::setItemValue(QAxObject* pRange,QVariant value){
    try {
