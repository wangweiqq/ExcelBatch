...
public:
	...
	//批量查询数据startRow起始行，startCols起始列，endRow终止行，endCols终止列
	QVariantList getRangeValues(QAxObject* pSheet,int startRow,int startCols,int endRow,int endCols);
	//批量插入数据startRow起始行，startCols起始列，endRow终止行，endCols终止列
	bool setRangeValues(QAxObject* pSheet, int startRow, int startCols, int endRow, int endCols, QList<QList<QVariant> >& values);
    //根据行列值设置单元格值
    ...
    /*设置单元格值*/
    bool setItemValue(QAxObject* pRange,QVariant value);
    /*设置单元格背景色*/
